...
$logFile = "$logDir\\log_weekly_$dateLog.log";
...
if($environment=="prod"){
	$subentro_custom_name = getenv('SUBENTRO_PROD');
	$switchin_custom_name = getenv('SWITCHIN_PROD');
	$voltura_custom_name = getenv('VOLTURA_PROD');
} else {
	$subentro_custom_name = getenv('SUBENTRO_TEST');
	$switchin_custom_name = getenv('SWITCHIN_TEST');
	$voltura_custom_name = getenv('VOLTURA_TEST');
}

// Definisco gli URL da cui attingere ai CSV
$subentro = getenv('URL_SUBENTRO');
$switchin = getenv('URL_SWITCHIN');
$voltura = getenv('URL_VOLTURA');

// Scarico i file csv e li salvo in tmp
logMessage("Download del file dei subentri > $subentro_custom_name", $logFile);
file_put_contents("$uploadFolder\\$subentro_custom_name", file_get_contents($subentro));

logMessage("Download del file degli switch > $switchin_custom_name", $logFile);
file_put_contents("$uploadFolder\\$switchin_custom_name", file_get_contents($switchin));

logMessage("Download del file delle volture > $voltura_custom_name", $logFile);
file_put_contents("$uploadFolder\\$voltura_custom_name", file_get_contents($voltura));
...
if ($sizeCheck == 0 && $filesUploaded == 0) {
    $subject = getenv('SUBJECT_WEEKLY_OK');
	...
	$subject = getenv('SUBJECT_WEEKLY_ERR');
